...
export const getMulti = createAsyncThunk(
  "GET_MULTIES",
  async (keyword: string) =>
    (
      await instance.get(
        `/search/movie?&query=${keyword}&page=1&include_adult=false`
      )
    ).data
);

export const getWhatsPopulars = createAsyncThunk(
  "GET_WHATS_POPULARS",
  async () => (await instance.get(`/movie/popular?&page=1`)).data
);

export const getFreeToWatches = createAsyncThunk(
  "GET_FREE_TO_WATCHES",
  async () => (await instance.get(`/list/2`)).data
);

export const getNowPlayings = createAsyncThunk(
  "GET_NOW_PLAYINGS",
  async () => (await instance.get(`/movie/now_playing?&page=1`)).data
);

export const getTrends = createAsyncThunk(
  "GET_TRENDS",
  async () => (await instance.get(`/trending/all/day`)).data
);

export const getMovies = createAsyncThunk(
  "GET_MOVIES",
  async ({ page, div }: any) =>
    (await instance.get(`/movie/${div}?page=${page}`)).data
);

export const searchMovies = createAsyncThunk(
  "SEARCH_MOVIES",
  async (keyword: string) =>
    (await instance.get(`/search/movie?query=${keyword}`)).data
);

export const getMovie = createAsyncThunk(
  "GET_MOVIE",
  async (id: string) => (await instance.get(`/movie/${id}`)).data
);

export const getTvShows = createAsyncThunk(
  "GET_TV_SHOWS",
  async ({ page, div }: any) =>
    (await instance.get(`/tv/${div}?S&page=${page}`)).data
);

export const searchTvShows = createAsyncThunk(
  "SEARCH_TV_SHOWS",
  async (keyword: string) =>
    (await instance.get(`/search/tv?query=${keyword}`)).data
);

export const getTvShow = createAsyncThunk(
  "GET_TV_SHOW",
  async (id: string) => (await instance.get(`/tv/${id}`)).data
);

export const getPeoples = createAsyncThunk(
  "GET_PEOPLES",
  async (page: string = "1") =>
    (await instance.get(`/person/popular?&page=${page}`)).data
);
...
export const searchPeoples = createAsyncThunk(
  ...
  async (keyword: string) =>
    (
      await instance.get(
        `/search/person?&page=1&include_adult=false&query=${keyword}`
      )
    ).data
);
...
export const getPeople = createAsyncThunk(
  "GET_PEOPLE",
  async (id: string) => await (await instance.get(`/person/${id}`)).data
);
...
export const getExternalIds = createAsyncThunk(
  ...
  async (id: string) =>
    await (
      await instance.get(`/person/${id}/external_ids`)
    ).data
);
...
export const getMovieCredits = createAsyncThunk(
  ...
  async (id: string) =>
    await (
      await instance.get(`/person/${id}/movie_credits`)
    ).data
);

export const getCombinedCredits = createAsyncThunk(
  "GET_COMBINDED_CREDITS",
  async (id: string) =>
    (await instance.get(`/person/${id}/combined_credits`)).data
);
...
export const getReviews = createAsyncThunk(
  ...
  async (id: string) =>
    await (
      await instance.get(`/movie/${id}/reviews?page=1`)
    ).data
);
...
const reducer: Reducer<StateProps, AnyAction> = createReducer(
  initialState,
  (builder) => {
    builder
      .addCase(getMulti.fulfilled, (state, { payload }) => {
        state.multi = payload;
      })
